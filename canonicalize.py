...
UNKNOWN_FEATURE_MSG = "Unknown feature: %r (file %s, line %d)"

FEATURE_CONFLICT_MSG = "Feature conflict: %r and %r (file %s, line %d)"
...
def main() -> None:
    ...
    parser.add_argument(
        "--fail-on-unknown-tags", action="store_true",
        help="Fail on unknown features, otherwise skip such lines and leave them as they are"
    )
    parser.add_argument(
        "--fail-on-tag-conflict", action="store_true",
        help="Fail on conflicting features, otherwise skip such lines and leave them as they are"
    )
    ...
    with open(args.input_path, "r") as source:
        ...
        # things here.
        logger = logging.error if args.fail_on_unknown_tags else logging.warning
        ...
        for (n, (lemma, inflection, features_str)) in enumerate(tsv_reader, 1):
            ...
            for feature in features:
                ...
                if current:
                    logger(
                        FEATURE_CONFLICT_MSG,
                        current,
                        feature,
                        args.input_path,
                        n,
                    )
                    if args.fail_on_tag_conflict:
                        exit(1)
                    else:
                        has_fails = True
                        break
